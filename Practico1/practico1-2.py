...
TARGET = 42          # Número entero que queremos representar en binario


# ---------------------------
# Función auxiliar: convertir lista de bits a número entero
# ---------------------------

def binarioToEntero(individual) -> int:

    result: int = 0
    individual = list(reversed(individual))
    i: int = 0
    n = len(individual)
...
def fitness(individual):
    value = binarioToEntero(individual)
    return -abs(value - TARGET) # Cuanto más cerca de TARGET mejor (máximo fitness = cuando value == TARGET)
...
print(f"Valor entero: {binarioToEntero(best)}")
